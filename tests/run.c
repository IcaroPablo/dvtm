...
/* Distinct foreground colours currently on screen. The point is the extended
 * colour-pair path, which only matters above 256. */
static int screen_distinct_fg(void)
{
	static unsigned long seen[4096];
	int n = 0;
	VTermPos p;
	for (p.row = 0; p.row < ROWS; p.row++) {
		for (p.col = 0; p.col < COLS; p.col++) {
			...
			if (!VTERM_COLOR_IS_RGB(&c.fg))
				continue;
			key = ((unsigned long)c.fg.rgb.red << 16) |
			      ((unsigned long)c.fg.rgb.green << 8) |
			      (unsigned long)c.fg.rgb.blue;
			for (i = 0; i < n; i++)
				if (seen[i] == key)
					break;
			if (i == n && n < (int)(sizeof seen / sizeof *seen))
				seen[n++] = key;
		}
	}
	return n;
...
static void t_truecolor(void)
{
	...
	check("truecolor: ':' subparameter separator survives to the outer terminal",
	      ...
	      "the subparameter separator was not parsed");
...
static void t_manycolors(void)
{
	...
	settle(500);
	n = screen_distinct_fg();
	if (n > 255) {
		ok("more than 255 distinct colours on screen");
	} else {
		char why[128];
		snprintf(why, sizeof why,
		         "only %d distinct rgb foregrounds; the extended pair path did not engage", n);
		fail("more than 255 distinct colours on screen", why);
	}
	reap();
}
